 ...
 */
export const editString = (str, toAdd, caretStart, caretEnd = caretStart) => {
  const firstHalf = str.slice(0, caretStart);
  const secondHalf = str.slice(caretEnd, str.length);
  return `${firstHalf}${toAdd}${secondHalf}`;
}

export const formatThousands = (value, options) => {
  const startIndex = value.indexOf(options.decimal) > -1
    ? value.indexOf(options.decimal) - 1
    : value.length - 1;
  const endIndex = 0;
  ...
  for (i; i > endIndex; i--, j++) {
    ...
    if (j % 3 === 0) {
      value = editString(value, options.thousands, i);
    }
  }
 ...
 */
export const partialFormat = (value, options) => {
  value = value.replace(new RegExp(`[${options.thousands}]`, 'g'), '');
  value = removeleadingZeros(value, options);
  value = removeExtraDecimals(value, options);
  value = formatThousands(value, options);
